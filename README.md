...
- **Dashboard**:`/dashboard` 同时提供历史记录与每日行动视图，保存岗位、链接、精确发布日期、DeepSeek JD 摘要、匹配分、使用的简历与事件历史。主界面状态包括待投递、已提交、被拒绝、电话面试、正式面试、有 Offer 和岗位已失效；顶部统计卡片可点击并直接联动筛选，每张岗位卡片可确认已提交或转换状态，每张合格岗位卡片还可直接发起一次 ApplyPilot 投递。
- **评分规则**:`/scoring-rules` 直接展示并编辑 ApplyPilot Skill 实际交给 DeepSeek 的完整规则。每次人工保存会先备份旧版本；普通运行不会自行改写规则。
- **自动搜索任务**:设置页可开关增量/每日全量搜索，调整增量间隔和全量运行时间。普通“保存设置”只保存计划；点击“保存并应用到 macOS 定时任务”才会更新本机 LaunchAgents。
- **一键运行**:页面上三个按钮——只抓取 / 抓取+打分 / 用缓存重新打分,后台跑,页面上能看实时日志。API key 可以直接在页面填(只在这次运行时用,不写进任何文件),也可以照旧用 `export`。
- **统一 Dashboard**:`/dashboard` 已合并结果界面与申请记忆：使用结果页的卡片 UI 和 accent color，按匹配分从高到低排列，并可按最低分、最近同步、今日行动（待处理）、今日提交、状态、担保、来源和新鲜度筛选。“最近同步”是最新一次运行写入的岗位；“今日行动（待处理）”是当前仍需处理的岗位；“今日提交”按 Melbourne 本地日期和有证据的 `submitted_at` 统计。首屏渐进显示 40 条；运行完成后已打开的 Dashboard 会自动刷新，页面也会按当前时间重新计算新鲜度标签和筛选，不会把旧的 `24 小时内` 状态永久保留。所有卡片共用一个状态弹窗，避免为每条记录重复渲染表单。用户已经在外部平台确认成功后，卡片上的“确认已提交”点一次即可，不需要再输入文字、URL 或勾选额外确认；确认提交和状态转换都只局部更新对应卡片及统计，不触发整页刷新。系统会自动同步主账本、事件历史和已有 attempt。投递按钮直接启动单岗位执行，进度在原卡片内刷新；内部 attempt CSV 仅用于去重和审计，不作为用户队列。工作年限资格说明是本地可重算字段：同步时会使用当前 JD 重新计算并自动纠正旧解析结果，不需要再次调用 DeepSeek。
...
**验证状态**:当前 429 项离线检查全部通过。2026-07-24 已完成一次真实 DeepSeek 批量评分并取得 243/243 个有效结果；漏项会只重试缺失岗位一次，不由当前 Agent 补造分数。
...
**工作年限只在候选人资格上下文中识别。** `application_policy.py` 不会再把“公司成立 25 年”“57 年连续盈利”“毕业于最近 2 年”或“1 年固定期限合同”当成候选人的工作经验要求；资格列表中的 `3+ years of experience`、`Minimum 7 years in product management` 等仍会被识别。LinkedIn Markdown 中的转义范围（例如 `2\-5 years`）会先规范化并取下限。当前原始 JD 快照存在时，Dashboard 会在同步时重算资格说明；快照已经不存在且明显来自旧误判的记录会显示为“需人工确认”，不会继续展示虚假的超高年限。
...
截至 2026-08-09，离线回归与此前使用 `probe.py` 的真实环境验证结果如下（macOS、Python 3.14、python-jobspy 1.1.82）：

| 部分 | 状态 |
|---|---|
| 薪资解析 / 去重 / 归一化 / 签证过滤 / 担保信号加扣分 | ✅ 78 项单元测试 |
| SEEK v5 字段解析 + 日期过滤 | ✅ 75 项单元测试(用实测结构造的样本) |
| 全链路接线(去重→过滤→打分→单文档汇总→生成→写文件) | ✅ 36 项端到端离线测试,桩 LLM |
| Candidate Profile / 申请策略 / Dashboard / Web 路由 | ✅ 240 项离线测试，包含工作年限误判回归场景 |
| DeepSeek provider 初始化/报错逻辑 | ✅ 离线覆盖；2026-07-24 真实批量调用取得 243/243 个有效结果 |
...
| 真实 LLM 打分质量 | ⚠️ API 与结构化返回已验证；匹配质量仍需人工抽查 |
...
```bash
python test_pipeline.py             # 78 项:薪资解析、归一化、去重、签证过滤、担保信号加扣分
python test_seek_parse.py           # 75 项:SEEK v5 字段解析、降级路径、日期过滤
python test_e2e_offline.py          # 36 项:全链路接线,桩数据 + 桩 LLM,含单文档汇总
python test_application_system.py   # 240 项:Candidate Profile、申请策略、Dashboard 和 Web 路由
